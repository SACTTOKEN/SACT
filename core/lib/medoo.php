...
class Medoo
{
	...
	{
		if(strpos($string, '[SIGNED]') !== false){
			$string=str_replace("[SIGNED]","",$string);
			return 'CONVERT(' . $string . ',SIGNED)';
		}
				...
				if (isset($match[ 'operator' ]))
				{
					...
					elseif ($operator === '<>' || $operator === '><')
					{
						if ($type === 'array')
						{
							...
							$data_type = (is_numeric($value[ 0 ]) && is_numeric($value[ 1 ])) ? PDO::PARAM_STR : PDO::PARAM_STR;
