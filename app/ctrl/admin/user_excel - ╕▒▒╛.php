...
class user_excel extends BaseController{
    ...
    public function excel_in(){
        ...
        $filepath = post('filename','');
        $filepath =  IMOOC.str_replace('/api/', 'public/', $filepath);
        $iden_ar = post(['nickname','username','rating','money','amount','integral','created_time','reg_ip','openid','tid']);
        ...
        $sql2 = "update user as a set a.tid=IFNULL((select id from user where username=a.tid_cn),0)";
