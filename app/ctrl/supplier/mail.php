...
class mail extends BaseController{
	...
	{
		$mail_ar=$this->mail_M->have(['sid'=>$GLOBALS['user']['id']]);
		if(empty($mail_ar)){
			$this->mail_M->save(['sid'=>$GLOBALS['user']['id'],'title'=>'快递公司','title_en'=>'快递编码']);
		}
		$res=$this->mail_M->have(['sid'=>$GLOBALS['user']['id']]);
	...
	{
		$id=$this->mail_M->have(['sid'=>$GLOBALS['user']['id']],'id');
